...
let ConteoCorreos=document.getElementById('ConteoCorreos')
let btnOpen=document.getElementById('btnOpen')

let GetState=function(){
  console.log("Pruebas")
}
//let localStorage=localStorage.setItem("FnGetState",GetState)
btnOpen.addEventListener('click',()=>{
  ...
  doc.write(
      "<html><head>"
      + "<script src='build/fixter.js'></script>"
      + "</head><body>"
      + "<h1>sssss</h1>"
      + "</body></html>"
  );
...
function countEmails(){
  ConteoCorreos.innerHTML="";
  let emails = store.getState().emails
  ConteoCorreos.innerHTML=`No de registro: ${emails.length}`
